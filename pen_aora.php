        ...
        <form method="post" action="pen_voucher_edit.php">
            <div>
                ...
                <?php
                    ...
                    if ($tanggal1==$tanggal2)
                        {
                    $query=mysql_query("select lokasi.lokasi,lokasi.loket as looket, IFNULL(sum(aora.tagihan),0) as tagihan,IFNULL(sum(aora.fee_ca),0) as fee_ca,IFNULL(sum(aora.fee_finnet),0) as fee_finnet,IFNULL(sum(aora.kewajiban),0) as kewajiban from lokasi left join aora on  lokasi.loket=aora.loket AND tanggal='$tampil_date' group by lokasi.loket");
                        }
                        else
                        {
