        ...
        <form method="post" action="pen_voucher_edit.php">
            <div>
                ...
                <?php
                    ...
                    if ($tanggal1==$tanggal2)
                        {
                    $query=mysql_query("select lokasi.lokasi,lokasi.loket as looket, voucher_tri.tanggal, voucher_tri.loket,  voucher_tri.denom as denom, 
                        sum(voucher_tri.total_kopeg) as total_kopeg1, voucher_tri.total_kopeg as total_kopeg2, count(voucher_tri.denom) as hitung  ,  voucher_tri.user, fee_ca*count(voucher_tri.denom) as fee_ca from lokasi left join voucher_tri on voucher_tri.user=lokasi.loket where tanggal='$tampil_date' or tanggal is NULL group by lokasi, denom");
                        }
                        else
                        {
